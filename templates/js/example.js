...
var baseHref = '/examples';
var exConfig = gallery_demos,
...
function createGalleryItem(id, config) {
    ...
    if (config.content) {
        var categoryContentDiv = $("<div class='category-content'></div>").appendTo(categoryLi);
        var content = $("<div class='box box-default color-palette-box' id='" + config.name + '-' + id + "'></div>");
        createGalleryCharts(config.content).appendTo(content);
...
function createGalleryChart(example) {
    var target = "./editor.html",
        title = example.name,
        href = example.file ? example.file : "",
        img = example.img ? '.'+example.img : "";
    ...
    var thumb = $("<img class='chart-area' src='" + img + "' style='display: inline'>");
